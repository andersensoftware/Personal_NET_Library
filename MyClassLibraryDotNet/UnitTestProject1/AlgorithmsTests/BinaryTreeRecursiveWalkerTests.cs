...
namespace UnitTest
{
    ...
    public class BinaryTreeRecursiveWalkerTests
    {
        ...
        private static void TestPreOrderTraversal()
        {
            ...
            Assert.IsTrue(list.ToArray() == new int[] { 3, 5, 7, 10, 13, 15 }, "Wrong traversal, expected InOrder enumeration of tree!");
        }
        ...
        private static void TestInOrderTraversal()
        {
            ...
            Assert.IsTrue(list.ToArray() == new int[] { 10, 5, 3, 7, 15, 13, 17 }, "Wrong traversal, expected InOrder enumeration of tree!");
        }
        ...
        private static void TestPostOrderTraversal()
        {
            ...
            Assert.IsTrue(list.ToArray() == new int[] { 3, 7, 5, 13, 17, 15, 10 }, "Wrong traversal, expected InOrder enumeration of tree!");
        }
        ...
        private static void TestContainReturnsTrue()
        {
            ...
            foreach (var value in values)
                Assert.IsTrue(BinaryTreeRecursiveWalker.Contains(_root, value, preOrder),
                    "Wrong boolean returned, expected True from Contains");

            foreach (var value in values)
                Assert.IsTrue(BinaryTreeRecursiveWalker.Contains(_root, value, inOrder),
                    "Wrong boolean returned, expected True from Contains");

            foreach (var value in values)
                Assert.IsTrue(BinaryTreeRecursiveWalker.Contains(_root, value, postOrder),
                    "Wrong boolean returned, expected True from Contains");
        }
        ...
        private static void TestContainReturnsFalse()
        {
            ...
            foreach (var value in values)
                Assert.IsFalse(BinaryTreeRecursiveWalker.Contains(_root, value, preOrder),
                    "Wrong boolean returned, expected False from Contains");

            foreach (var value in values)
                Assert.IsFalse(BinaryTreeRecursiveWalker.Contains(_root, value, inOrder),
                    "Wrong boolean returned, expected False from Contains");

            foreach (var value in values)
                Assert.IsFalse(BinaryTreeRecursiveWalker.Contains(_root, value, postOrder),
                    "Wrong boolean returned, expected False from Contains");
        }
        ...
        private static void TestBinarySearchReturnsTrue()
        {
            ...
            foreach (var value in values)
                Assert.IsTrue(BinaryTreeRecursiveWalker.BinarySearch(_root, value, preOrder),
                    "Wrong boolean returned, expected True from Contains");

            foreach (var value in values)
                Assert.IsTrue(BinaryTreeRecursiveWalker.BinarySearch(_root, value, inOrder),
                    "Wrong boolean returned, expected True from Contains");

            foreach (var value in values)
                Assert.IsTrue(BinaryTreeRecursiveWalker.BinarySearch(_root, value, postOrder),
                    "Wrong boolean returned, expected True from Contains");
        }
        ...
        private static void TestBinarySearchReturnsFalse()
        {
            ...
            foreach (var value in values)
                Assert.IsFalse(BinaryTreeRecursiveWalker.BinarySearch(_root, value, preOrder),
                    "Wrong boolean returned, expected False from Contains");

            foreach (var value in values)
                Assert.IsFalse(BinaryTreeRecursiveWalker.BinarySearch(_root, value, inOrder),
                    "Wrong boolean returned, expected False from Contains");

            foreach (var value in values)
                Assert.IsFalse(BinaryTreeRecursiveWalker.BinarySearch(_root, value, postOrder),
                    "Wrong boolean returned, expected False from Contains");
        }
