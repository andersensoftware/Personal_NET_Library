...
namespace UnitTest.DataStructuresTests
{
    ...
    public static class RedBlackTreeMapTests
    {
        ...
        public static void DoTest()
        {
            ...
            Assert.IsTrue(redBlackTree.Count == values.Length, "Expected the same number of items.");
            Assert.IsTrue(redBlackTree.Height < redBlackTree.Count, "Fail! Tree doesn't rebalance against sorted elements!");
            ...
            Assert.IsTrue(insert_duplicate_passed == false, "Fail! The tree doesn't allow duplicates");

            // Test find
            Assert.IsTrue(redBlackTree.Find(5).Key == 5, "Wrong find result!");
            Assert.IsTrue(redBlackTree.FindMin().Key == 1, "Wrong min!");
            Assert.IsTrue(redBlackTree.FindMax().Key == 10, "Wrong max!");
            ...
            Assert.IsTrue(true == threwKeyNotFoundError, "Expected to catch KeyNotFoundException.");
            ...
            // Assert count
            Assert.IsTrue(redBlackTree.Count == 10);

            // Assert existence and nonexistence of some items
            Assert.IsTrue(redBlackTree.Contains(1) == true);
            Assert.IsTrue(redBlackTree.Contains(3) == true);
            Assert.IsTrue(redBlackTree.Contains(999) == false);
            ...
            // Assert count
            Assert.IsTrue(redBlackTree.Count == 7);

            // Assert nonexistence of previously existing items
            Assert.IsTrue(redBlackTree.Contains(1) == false);
            Assert.IsTrue(redBlackTree.Contains(3) == false);
            ...
            // Assert count
            Assert.IsTrue(redBlackTree.Count == 6);

            // Assert nonexistence of old root's key
            Assert.IsTrue(redBlackTree.Contains(oldRootKey) == false);
        ...
        private static void AssetLevelsDifferentColors<TKey, TValue>(RedBlackTreeMap<TKey, TValue> redBlackTree) where TKey : IComparable<TKey>
        {
            ...
            for (int i = 0; i < levels.Count; ++i)
            {
                ...
                {
                    Assert.IsTrue(levels[i][j].Color == color);
