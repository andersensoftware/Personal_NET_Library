...
namespace UnitTest.DataStructuresTests
{
    ...
    public static class PriorityQueuesTest
	{
		...
		public static void DoTest ()
		{
            ...
            Assert.IsTrue(leastPriorityProcess.Id == process1.Id, "Wrong process!");

            sysProcesses.DequeueMin();

            leastPriorityProcess = sysProcesses.PeekAtMinPriority();
            Assert.IsTrue(leastPriorityProcess.Id == process5.Id, "Wrong process!");

            sysProcesses.DequeueMin();

            leastPriorityProcess = sysProcesses.PeekAtMinPriority();
            Assert.IsTrue(leastPriorityProcess.Id == process3.Id, "Wrong process!");

            sysProcesses.DequeueMin();

            leastPriorityProcess = sysProcesses.PeekAtMinPriority();
            Assert.IsTrue(leastPriorityProcess.Id == process6.Id, "Wrong process!");
