...
namespace UnitTest.DataStructuresTests
{
    ...
    public static class BinarySearchTreeMapTests
    {
        ...
        public static void DoTest()
        {
            ...
            Assert.IsTrue(bstMap.Count == values.Length, "Expected the same number of items.");
            ...
            Assert.IsTrue(bstMap.Count == values.Length + 1, "Expected the same number of items.");
           ...
           Assert.IsFalse(insert_duplicate_passed, "Fail! The tree doesn't allow duplicates");

            // Test find
            Assert.IsTrue(bstMap.Find(5).Key == 5, "Wrong find result!");
            Assert.IsTrue(bstMap.FindMin().Key == 0, "Wrong min!");
            Assert.IsTrue(bstMap.FindMax().Key == 10, "Wrong max!");
            ...
            Assert.IsTrue(threwKeyNotFoundError, "Expected to catch KeyNotFoundException.");

            // Assert count
            Assert.IsTrue(bstMap.Count == 11);

            // Assert existence and nonexistence of some items
            Assert.IsTrue(bstMap.Contains(1));
            Assert.IsTrue(bstMap.Contains(3));
           Assert.IsFalse(bstMap.Contains(999));
            ...
            // Assert count
            Assert.IsTrue(bstMap.Count == 8);

            // Assert nonexistence of previously existing items
           Assert.IsFalse(bstMap.Contains(1));
           Assert.IsFalse(bstMap.Contains(3));
            ...
            // Assert count
            Assert.IsTrue(bstMap.Count == 7);

            // Assert nonexistence of old root's key
           Assert.IsFalse(bstMap.Contains(oldRootKey));
