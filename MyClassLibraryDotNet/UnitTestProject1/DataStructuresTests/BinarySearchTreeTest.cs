...
namespace C_Sharp_Algorithms
{
    ...
    public static class BinarySearchTreeTest
    {
        ...
        public static void AssertTreeWithDuplicatesElements()
        {
            ...
            Assert.AreEqual(21, binarySearchTree.Count);

            // Test contains/find
            Assert.IsTrue(binarySearchTree.Contains(10), "Wrong element.");

            // Test find all
            var list = binarySearchTree.FindAll(element => element > 15).ToList();
            Assert.IsTrue(list.Count == 7, "Wrong FindAll result!");

            // test sort
            List<int> sortedList = binarySearchTree.ToList();
            for (int i = 1; i < sortedList.Count; ++i)
                Assert.IsTrue(sortedList[i - 1] <= sortedList[i], "BST sort is wrong!");

            // ASSERT MIN ITEM
            Assert.IsTrue(binarySearchTree.FindMin() == -1, "Min is wrong.");

            // ASSERT MAX ITEM
            Assert.IsTrue(binarySearchTree.FindMax() == 39, "Max is wrong.");
            ...
            Assert.IsTrue(binarySearchTree.FindMax() == 30, "Max is wrong.");
            ...
            // ASSERT MIN
            Assert.IsTrue(binarySearchTree.FindMin() == 7, "Min is wrong.");
            ...
            Assert.IsTrue(binarySearchTree.FindMax() == 20, "Max is wrong.");
            ...
            enumerator.MoveNext();
            enumerator.MoveNext();
            Assert.IsTrue(enumerator.Current == 8, "Wrong in-order enumeration.");
        }
        ...
        public static void AssertTreeWithUniqueElements()
        {
           ...
           Assert.IsFalse(inserting_duplicates_passed, "Fail! Tree doesn't allow duplicates");
            ...
            Assert.IsTrue(inserting_unique_passed, "Fail! Inserting unique elements should pass!");

            // ASSERT COUNT
            Assert.AreEqual(binarySearchTree.Count, values.Length);
        }
